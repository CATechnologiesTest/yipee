...
;; json map -> [:map [<key> <value type>] ...]
;; open map (such as labels) -> [:open-map <key type> <value type>]
...
;; optional value -> list
;;
;; e.g. metadata = [:map ([:annotations [:open-map :keyword :string]])
;;                       ([:labels [:open-map :keyword :string]])
;;                       [:name :string]
;;                       ([:namespace :string])]
...
(defflat k8s-service
  ...
  [:name :string]
  [:metadata [:map
              ([:annotations [:open-map :keyword :string]])
              ([:labels [:open-map :keyword :string]])
              [:name :string]
              ([:namespace :string])]]
  [:selector :json]
